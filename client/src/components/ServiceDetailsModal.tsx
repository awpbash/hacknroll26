...
const getServiceDetails = (service: CloudService, category: ServiceCategory) => {
  ...
  if (category === 'compute') {
    if (service.name.includes('EC2') || service.name.includes('VM') || service.name.includes('E2')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/hr'), description: 'Hourly billing with reserved instance discounts' },
    ...
    if (service.name.includes('RTX') || service.name.includes('A100') || service.name.includes('H100') || service.name.includes('A6000')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/hr'), description: 'Premium GPU pricing with high performance' },
  ...
  if (category === 'serverless') {
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/req'), description: 'Pay per execution with generous free tier' },
  ...
  if (category === 'storage') {
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/GB'), description: 'Pay per GB stored plus data transfer costs' },
  ...
  if (category === 'database') {
    if (service.name.includes('MongoDB')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/mo'), description: 'Tiered pricing with free tier available' },
    ...
    if (service.name.includes('DynamoDB') || service.name.includes('Cosmos') || service.name.includes('Firestore')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/op'), description: 'Pay per request with reserved capacity options' },
    ...
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/hr'), description: 'Instance pricing plus storage costs' },
  ...
  if (category === 'networking') {
    if (service.name.includes('Load Balancer') || service.name.includes('ELB')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/hr'), description: 'Hourly charge plus data processing fees' },
    ...
    if (service.name.includes('CDN') || service.name.includes('CloudFront')) {
      return {
        ...
        evaluation: {
          pricing: { value: formatPrice(service.baseCost, '/GB'), description: 'Data transfer pricing with regional variations' },
  ...
  if (category === 'cache') {
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/hr'), description: 'Instance pricing based on memory size' },
  ...
  if (category === 'messaging') {
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/req'), description: 'Pay per request with generous free tier' },
  ...
  if (category === 'ai') {
    return {
      ...
      evaluation: {
        pricing: { value: formatPrice(service.baseCost, '/token'), description: 'Pay per token or inference request' },
